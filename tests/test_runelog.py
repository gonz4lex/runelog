...
def test_logging_functions(tracker):
    ...
    with tracker.start_run(experiment_id=exp_id) as run_id:
        ...
        param_path = os.path.join(tracker._get_run_path(), "params", "learning_rate.json")
...
def test_log_artifact_and_model(tracker):
    ...
    with tracker.start_run(experiment_id=exp_id) as run_id:
        ...
        logged_artifact_path = os.path.join(tracker._get_run_path(), "artifacts", "dummy_artifact.txt")
        assert os.path.exists(logged_artifact_path)

        # Test model logging
        model = MockModel()
        tracker.log_model(model, "test_model.pkl")
        logged_model_path = os.path.join(tracker._get_run_path(), "artifacts", "test_model.pkl")
