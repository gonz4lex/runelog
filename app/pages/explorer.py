...
def show_actions(run_id: str, details):
    model_artifact = next((art for art in details.get("artifacts", []) if art.endswith(".pkl")), None)
    ...
    if model_artifact:
        ...
        with c2:
            if st.button("Add to Registry"):
                if model_name:
                    try:
                        version = tracker.register_model(run_id, model_artifact, model_name)
                        st.success(
            ...
            with st.expander(f"📄 {artifact_name}"):
                try:
                    ...
                    if extension in ['.png', '.jpg', '.jpeg', '.gif']:
                        ...
                        st.info(f"Preview is not available for this file type ({extension}).")
...
if selected_experiment_id:
    ...
    if not results_df.empty:
        results_df.insert(results_df.shape[1], "select", False)

        edited_df = st.data_editor(
            results_df,
            column_config={"select": st.column_config.CheckboxColumn(required=True, width="small")},
            disabled=results_df.columns.drop("select"),
            hide_index=False,
        )

        selected_rows = edited_df[edited_df.select]
        selected_run_ids = selected_rows.index.tolist()

        if len(selected_run_ids) >= 2:
            selected_run_ids_str = ", ".join([f"`{id}`" for id in selected_run_ids])
            st.markdown(f"### Comparing Runs: {selected_run_ids_str}")
            
            metric_columns = [col for col in selected_rows.columns if not col.startswith("param_") and col != "select"]
            
            if not metric_columns:
                st.warning("No metrics found to compare for the selected runs.")
            else:
                selected_metric = st.selectbox("Select a metric to compare:", options=metric_columns)

                chart_data = selected_rows[[selected_metric]]
