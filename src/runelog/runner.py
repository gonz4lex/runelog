...
def _run_single_experiment(model, X, y, validation_config: dict, evaluation_config: dict) -> dict:
    ...
    if validation_config['strategy'] == 'cross_validation':
        ...
        for metric_name in metrics_to_compute:
            metric_func = getattr(importlib.import_module("sklearn.metrics"), metric_name)
        ...
        scores = cross_validate(model, X, y, cv=cv_params.get('n_splits', 5), scoring=scoring)
        
        # Average the scores from all folds
        avg_scores = {key.replace("test_", ""): value.mean() for key, value in scores.items() if key.startswith('test_')}
        return avg_scores
    ...
    # Split Strategy
        ...
        raise ValueError(f"Unknown validation strategy: {validation_config['strategy']}. Choose either 'test_train_split' or 'cross_validation'.")
...
def run_sweep(config_path: str, progress_handler=None):
    ...
    handler(f"Dataset '{input_dataset['source']}' loaded successfully. Shape: {X.shape}")
    ...
    for run_config in config.get("runs", []):
        ...
        with tracker.start_run(experiment_id=exp_id):
            ...
            # Run and get metrics
            metrics = _run_single_experiment(model, X, y, config['validation'], config['evaluation'])
